...
namespace BitSharp.Storage.MongoDB
{
    public class KnownAddressStorage : IBoundedStorage<NetworkAddressKey, NetworkAddressWithTime>
    {
        ...
        public KnownAddressStorage(MongoDBStorageContext storageContext)
        {
            ...
            this.collection = new MongoByteCollection(storageContext.Database, this.name);
        }
        ...
        {
            get { return this.collection.Keys.Select(x => DecodeKey(x)); }
        }

        public IEnumerable<NetworkAddressWithTime> Values
        {
            get { return this.collection.Values.Select(x => DecodeValue(x)); }
        }

        public bool ContainsKey(NetworkAddressKey networkAddressKey)
        {
            return this.collection.ContainsKey(EncodeKey(networkAddressKey));
        }
        ...
        public bool TryGetValue(NetworkAddressKey networkAddressKey, out NetworkAddressWithTime networkAddressWithTime)
        {
            ...
            if (this.collection.TryGetValue(EncodeKey(networkAddressKey), out networkAddressWithTimeBytes))
            {
        ...
        public NetworkAddressWithTime this[NetworkAddressKey networkAddressKey]
        {
            get
            {
                return DecodeValue(this.collection[EncodeKey(networkAddressKey)]);
            }
            set
            {
                this.collection[EncodeKey(networkAddressKey)] = EncodeValue(value);
            }
        }
